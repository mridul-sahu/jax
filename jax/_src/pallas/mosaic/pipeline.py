...
when = helpers.when
Squeezed = core.Squeezed
Indirect = core.Indirect
Element = core.Element
BoundedSlice = core.BoundedSlice
Blocked = core.Blocked
BlockSpec = core.BlockSpec
BlockDim = core.BlockDim
...
def _get_block_shape(spec: BlockSpec) -> tuple[int, ...]:
...
class BufferedRefBase:
  ...
  def with_spec(self, spec: BlockSpec) -> BufferedRefBase:
...
class BufferedRef(BufferedRefBase):
  ...
  """
  _spec: BlockSpec = dataclasses.field(metadata=dict(static=True))
  ...
  def create(
    ...
    if buffer_memory_space not in (SMEM, VMEM, HBM, HOST):
      raise ValueError(
  ...
  def with_spec(self, spec: BlockSpec) -> BufferedRef:
...
def _normalize_specs(specs: Any) -> tuple[BlockSpec, ...]:
